...
Sebelum distribusi, cek panjang extradata dengan Python:

python3 -c "
extradata = '0x...'  # paste extradata kamu di sini
total = len(extradata)
...
# Simpan genesis.json ke direktori blockchain
# (copy dari VM-1 atau download dari shared storage)
...
# Jalankan di VM-1 (10.34.100.182)
geth \
...
# Jalankan di masing-masing VM (ganti ADDR dan IP sesuai node)
geth \
  --datadir ~/blockchain/geth \
  --networkid 20260315 \
  --port 30303 \
...
# Cek daftar signer/validator aktif:
clique.getSigners()
